...
app = Flask(__name__)
# 配置摄像头
video_task = camera.Video_task(0,240,320)
# 配置串口
...
def get_gamepad():
    global crab,covering
    if request.method == 'GET':
        throw , ws, ad, rotate, shape, cover, drill = 0, 0, 0, 0, 0, 0 ,0
        
        button_0 = int(request.args.get('button_0'))
        button_2 = int(request.args.get('button_2'))
        if button_0 == button_2:
            drill = 0
        else:
            if button_0 == 1:
                drill = 1
        ...
        GamepadControl([throw,ws,ad,rotate,crab,cover,drill])
    return "ok"
