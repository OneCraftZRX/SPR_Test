...
def non_max_suppression(
    ...
    xc = prediction[:, 4:mi].amax(1) > conf_thres  # candidates,[1,8400],通过筛选的为true，否则为false
    ...
    t = time.time()
    output = [torch.zeros((0, 6 + nm), device=prediction.device)] * bs  #the list to save the result

    #apply the nms
    for xi, x in enumerate(prediction):  # image index, image inference,single image→xi=0
        # Apply constraints
        # x[((x[:, 2:4] < min_wh) | (x[:, 2:4] > max_wh)).any(1), 4] = 0  # width-height

        x = x.transpose(0, -1)[xc[xi]]  # confidence,运行完之后x的大小是[45,84]]
